...
export const Select = ({label, options, width="w-full", onChange, disabled = false}: SelectProps) => (
  <div className={`pb-3 ${width}`}>
    <p className="text-left mb-2">{label}</p>
    <select onChange={onChange} className={`rounded-3xl w-full py-2 px-4 ${disabled ? 'bg-medium-light-blue' : 'bg-light-blue'} border-none outline-none focus:outline-none`} disabled={disabled}>
      {options ? (
